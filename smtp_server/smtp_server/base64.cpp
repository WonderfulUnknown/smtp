...
string Base64::Encode(const unsigned char * str, int bytes) {
	...
	current = str;
	while (bytes > 2) {
		result += encode_table[current[0] >> 2];
		result += encode_table[((current[0] & 0x03) << 4) + (current[1] >> 4)];
		result += encode_table[((current[1] & 0x0f) << 2) + (current[2] >> 6)];
		result += encode_table[current[2] & 0x3f];
	...
	if (bytes > 0)
	{
		result += encode_table[current[0] >> 2];
		if (bytes % 3 == 1) {
			result += encode_table[(current[0] & 0x03) << 4];
			result += "==";
		}
		else if (bytes % 3 == 2) {
			result += encode_table[((current[0] & 0x03) << 4) + (current[1] >> 4)];
			result += encode_table[(current[1] & 0x0f) << 2];
			result += "=";
		}
	}
	return result;
